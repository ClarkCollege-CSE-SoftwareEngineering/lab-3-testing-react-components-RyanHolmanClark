...
describe('TaskItem', () => {
    ...
    describe('rendering', () => {
        it('displays the task title', () => {
            ...
            // Using getByText - appropriate here because the title IS the contest users see
            expect(screen.getByText('Buy groceries')).toBeInTheDocument();
        })

        it('show checkbox as unched for incomplete tasks', () => {
            const task = createTask({ completed: false });
            render(
                <TaskItem task={task} onToggle={vi.fn()} onDelete={vi.fn()} />
            )

            // Using the getByRole with name - the preferred query method
        ...
        it('shows checkbox as checked for competed tasks', () => {
    ...
    describe('interactions', () => {
        it('calls onToggle with taks id when checkbox is clicked', async () => {
        ...
        it('calls onDelete with task id when delete button is clicked', async () => {
            ...
            const task = createTask({ id: 'task-123', title: 'Task to delete' });
